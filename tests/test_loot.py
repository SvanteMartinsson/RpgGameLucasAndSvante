...
class PerEnemyUniqueTableTests(unittest.TestCase):
    ...
    # B42 loot policy: trash/standard enemies drop salvage + potions + common gear
    # but carry NO signature unique — uniques are reserved for elites/casters (and
    # the original signature-bearers). These are the drop-capable enemies with an
    # intentionally empty unique_table.
    NO_UNIQUE = {
        "wild_dog", "wild_stag", "giant_spider", "goblin_scrapper",
        "bog_leech", "ghoul", "grave_hound", "goblin_raider", "thornling",
    }
